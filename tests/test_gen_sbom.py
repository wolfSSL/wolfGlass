#!/usr/bin/env python3
"""Unit tests for the helpers in share/gen-sbom.

Run from the repo root:

    python3 -m unittest tests/test_gen_sbom.py
...
def _load_gen_sbom():
    ...
    target = here.parent / 'share' / 'gen-sbom'
...
class TestEnabledDepsCli(unittest.TestCase):
    ...
    def _run(self, *argv):
        import subprocess
        here = pathlib.Path(__file__).resolve().parent
        script = here.parent / 'share' / 'gen-sbom'
    ...
    def setUp(self):
        try:
            ...
            self.fail(
                ...
                'this in the unit job; see .github/workflows/selftest.yml.')
...
class TestCliMutualExclusion(unittest.TestCase):
    ...
    def test_srcs_file_matches_srcs_for_same_list(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            import subprocess
            here = pathlib.Path(__file__).resolve().parent
            script = str(here.parent / 'share' / 'gen-sbom')
            r1 = subprocess.run(
...
class TestCdxDepComponent(unittest.TestCase):
    ...
    def test_wolfssl_dep_component_carries_nvd_cpe(self):
        # The wolfBoot / wolfSSH case an integrator's NVD-driven scanner
        # needs: the embedded wolfSSL must be identifiable by CPE, not by
        # purl alone.
        _, comp = gs.cdx_dep_component(
            'wolfboot', '2.9.0', 'wolfssl', {'wolfssl': '5.9.1'})
...
#
# The verifier (provenance/bomsh_verify.py) is invoked by the bomsh CI job
...
bv_spec = importlib.util.spec_from_file_location(
    'bomsh_verify',
    pathlib.Path(__file__).resolve().parent.parent / 'provenance' / 'bomsh_verify.py')
